        ...
        <p>
          {
            node["$online"] == "true"
              ? "Online"
              : "This grow node is offline. Changes here will be applied after the connection is restored."
          }<br />
        </p>
        <hr />
        <CurrentGrowStage node={node} />
        <Button onClick={() => {
            this.props.navigator.pushPage({ component: GrowStageChanger });
          }}>Change stage</Button>
        <hr />
        <div>
          <h2>Stats</h2>
          <p>
            Temperature:{node["air_temp/degrees"]}°F<br />
            Water Level:{node["water_level/gallons"]}Gallons<br />
